...
@Service("k3sControllerPlanServiceImpl")
public class K3sControllerPlanServiceImpl extends AbstractK3sPlanService
{
    ...
    protected static final String CONTAINER_ROLE_LABEL = "com.lnjoying.jke.container.role";

    protected static final String CONTAINER_CONTROLLER_INIT = "com.lnjoying.jke.container.controller.init";
    ...
    {
        LOGGER.info("begin build k3s controller plan on node {}, for cluster {}",
                clusterNodePlanInfo.getClusterNodeInfo().getNodeId(), clusterNodePlanInfo.getClusterId());
        ...
        try
        {
            ...
            buildCertDeployerPlan(k3sConfig, certMap, clusterNodePlanInfo);
            buildServiceSideKickPlan(k3sConfig, clusterNodePlanInfo);
            buildControllerPlan(k3sConfig, clusterNodePlanInfo);
            buildHealthCheckController(k3sConfig, clusterNodePlanInfo);

        }catch (Exception e)
        {
            LOGGER.error("build k3s controller deploy plan for cluster:{} error:{}", clusterInnerInfo.getClusterId(), e);
    ...
    private int buildControllerPlan(K3sConfig k3sConfig, ClusterNodePlanInfo clusterNodePlanInfo)
    {
        LOGGER.info("build k3s controller plan for {}", clusterNodePlanInfo.getClusterId());

        ContainerPlan containerPlan = new ContainerPlan(DeployContainerName.JKE_K3S_CONTROLLER, k3sConfig.getRegistries());
        clusterNodePlanInfo.addContainerPlan(containerPlan);
        assembleK3sControllerPlan(k3sConfig, containerPlan, clusterNodePlanInfo);
    ...
    private int assembleK3sControllerPlan(K3sConfig k3sConfig, ContainerPlan containerPlan, ClusterNodePlanInfo clusterNodePlanInfo)
    {
        ...
        labels.put(CONTAINER_ROLE_LABEL, K3sRole.CONTROLLER);
        ...
        dockerInstParams.getHostConfig().setBinds(Arrays.asList(K3S_PATH_BIND, ETC_K3S_PATH_BIND));

        List<String> commandListArgs  = new ArrayList<>();
        commandListArgs.add("server");
        if (init)
        {
            commandListArgs.add("--cluster-init");
        }

        if (k3sContext.isDisableAgent())
        {
            commandListArgs.add("--disable-agent");
        }

        commandListArgs.add("--tls-san");
        commandListArgs.add("0.0.0.0");

        k3sContext.getDatastoreEndpoint().ifPresent(datastore -> {
            commandListArgs.add("--datastore-endpoint");
            commandListArgs.add(datastore);
        });

        k3sContext.getClusterCidr().ifPresent(clusterCidr -> {
            commandListArgs.add("--cluster-cidr");
            commandListArgs.add(clusterCidr);
        });

        k3sContext.getServiceCidr().ifPresent(serviceCidr -> {
            commandListArgs.add("--service-cidr");
            commandListArgs.add(serviceCidr);
        });

        k3sContext.getServiceNodePortRange().ifPresent(serviceNodePortRange -> {
            commandListArgs.add("--service-node-port-range");
            commandListArgs.add(serviceNodePortRange);
        });

        k3sContext.getClusterDns().ifPresent(clusterDns -> {
            commandListArgs.add("--cluster-dns");
            commandListArgs.add(clusterDns);
        });

        k3sContext.getClusterDomain().ifPresent(clusterDomain -> {
            commandListArgs.add("--cluster-domain");
            commandListArgs.add(clusterDomain);
        });
        ...
        k3sContext.getK3sKubeConfigOutput().ifPresent(config -> {
            envArgs.put(K3sOption.CONTAINER_ENV_KEY_K3S_KUBECONFIG_OUTPUT, config);
        });
    ...
    private int buildHealthCheckController(K3sConfig k3sConfig, ClusterNodePlanInfo clusterNodePlanInfo)
    {
        LOGGER.info("build k3s controller health checker plan for {}", clusterNodePlanInfo.getClusterId());
        ContainerPlan containerPlan = new ContainerPlan(DeployContainerName.JKE_HEALTH_CHECK_K3S, k3sConfig.getRegistries());
        clusterNodePlanInfo.addContainerPlan(containerPlan);
        String certPath = EnvUtil.getK3sCertPath(KeyCertName.K3S_KUBE_API);
        String keyPath = EnvUtil.getK3sKeyPath(KeyCertName.K3S_KUBE_API);
