...
public class Utils
{
    private static final String SAFE_CHARS = "ABCDEFGHJKMNPQRSTUVWXYZabcdefghjkmnpqrstuvwxyz23456789";
    ...
    public static String getRandomAlphanumericStr(Integer txtLength)
    {
        return RandomStringUtils.randomAlphanumeric(txtLength);
    }

    public static String generateSafeRandomString(int length) {
        return RandomStringUtils.random(length, SAFE_CHARS);
    }
    ...
    public static String diffDate(Date beginDate, Date endDate)
    {
        long yearMillisecond = 365L * 24 * 60 * 60 * 1000;
        long monthMillisecond = 30L * 24 * 60 * 60 * 1000;
        long dayMillisecond = 24L * 60 * 60 * 1000;
        long hourMillisecond = 60L * 60 * 1000;
        long minuteMillisecond = 60L * 1000;
        StringBuilder stringBuilder = new StringBuilder();
        long diff = endDate.getTime() - beginDate.getTime();
        if (diff > (yearMillisecond))
        {
            long year = diff / (yearMillisecond);
            stringBuilder.append(year).append(year > 1 ? " years " : " year ");
            diff = diff % (yearMillisecond);
        }
        if (diff > (monthMillisecond))
        {
            long month = diff / (monthMillisecond);
            stringBuilder.append(month).append(month > 1 ? " months " : " month ");
            diff = diff % (monthMillisecond);
        }
        if (diff > (dayMillisecond))
        {
            long day = diff / (dayMillisecond);
            stringBuilder.append(day).append(day > 1 ? " days " : " day ");
            diff = diff % (dayMillisecond);
        }
        if (diff > (hourMillisecond))
        {
            long hour = diff / (hourMillisecond);
            stringBuilder.append(hour).append(hour > 1 ? " hours ": " hour ");
            diff = diff % (hourMillisecond);
        }
        if (diff > (minuteMillisecond))
        {
            long minute = diff / (minuteMillisecond);
            stringBuilder.append(minute).append(minute > 1 ? " minutes " : " minute ");
        }
        return stringBuilder.toString();
    }
}
