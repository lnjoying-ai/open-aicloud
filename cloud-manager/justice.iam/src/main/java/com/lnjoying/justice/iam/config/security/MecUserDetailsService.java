...
public class MecUserDetailsService implements UserDetailsService
{
    ...
    public UserDetails loadUserByKey(String uniqueUserFlag) throws UsernameNotFoundException
    {
        ...
        if (!tblUserInfo.getIsAllowed() || tblUserInfo.getStatus() != UMStatus.ACTIVE) {
            throw new DisabledException(
                    "User is not allowed to login");
        }

        List<TblIamRole> rolePos = userRepository.getRolesByUserId(tblUserInfo.getUserId());
        ...
        if (isLocked) {
            LOGGER.info("username:{} have been locked.", tblUserInfo.getUserName());
        }
