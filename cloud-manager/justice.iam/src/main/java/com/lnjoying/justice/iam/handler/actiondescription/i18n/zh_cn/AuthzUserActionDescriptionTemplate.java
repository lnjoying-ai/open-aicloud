 ...
 */
public class AuthzUserActionDescriptionTemplate extends AbstractActionDescriptionTemplate
{

    private static final Logger LOGGER = LoggerFactory.getLogger(AuthzUserActionDescriptionTemplate.class);
    ...
    public Optional<String> getDescriptionTemplate()
    {
        ...
        switch (operationType)
        {
            case ResourceOperationTypeConstants.RESOURCE_CREATE:
                return Optional.of(USER_ASSIGN_AUTHORIZATION);
            case ResourceOperationTypeConstants.RESOURCE_UPDATE:
                return Optional.of(Descriptions.USER_REVOKE_AUTHORIZATION);
        }
    ...
    {
        String USER_ASSIGN_AUTHORIZATION = "用户 ${_TMP_USER_NAME} 添加授权策略: [${_TMP_POLICY_LIST}],  角色: [${_TMP_ROLE_LIST}]";
        String USER_REVOKE_AUTHORIZATION = "用户 ${_TMP_USER_NAME} 取消授权策略: [${_TMP_POLICY_LIST}],  角色: ${_TMP_ROLE_LIST}]";
    }
}
