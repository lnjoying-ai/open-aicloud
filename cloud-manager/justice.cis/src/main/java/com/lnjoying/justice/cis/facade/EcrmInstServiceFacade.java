 ...
 */
@Service
public class EcrmInstServiceFacade extends ClsInstServiceFacade
{
    ...
    public void updateInst4CreateEvent(EeInstDef.inst_status_desc statusInfo)
    {
        ...
        try
        {
            combRpcService.getEdgeResourceService().updateServiceAgentStatus(statusInfo.getInstId(), statusInfo.getInstStatus());
        }
        catch(Exception e)
        {
            e.printStackTrace();
    ...
    public void updateInstStatus(String instId, int status)
    {
        ...
        try
        {
            combRpcService.getEdgeResourceService().updateServiceAgentStatus(instId, status);
        }
        catch(Exception e)
        {
            e.printStackTrace();
