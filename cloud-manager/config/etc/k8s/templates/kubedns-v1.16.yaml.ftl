...
<#else>
  ...
  <#if ReverseCIDRs??>
  <#list ReverseCIDRs?keys as k>
        - --server=/${ReverseCIDRs[k]}/127.0.0.1#10053
  </#list>
  <#else>
...
<#if UpstreamNameservers??>
  upstreamNameservers: |
    <#list UpstreamNameservers as serverInfo>'${serverInfo}'<#if serverInfo_has_next>,</#if></#list>
</#if>
<#if StubDomains??>
  stubDomains: |
    <#if StubDomains??>${StubDomains}</#if>
</#if>