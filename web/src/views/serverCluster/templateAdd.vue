<template>
  <div style="margin-top: -20px">
    ...
    <!-- 集群信息结束-->
    <!-- 集群模板开始-->
    <div v-if="this.templateForm.cluster_type == 'k8s'">
      <add-template-versions ref="addTemplateVersions" />
    </div>
    <div v-if="this.templateForm.cluster_type == 'k3s'">
      <addK3sTemplateVersionsVue ref="addK3sTemplateVersions" />
    </div>
    <!-- 集群模板结束-->
    <!-- 集群选项 -->
...
import addK3sTemplateVersionsVue from "./addK3sTemplateVersions.vue";

export default {
  components: { addTemplateVersions, addK3sTemplateVersionsVue },
  mixins: [initData],
  computed: {
    ...mapGetters(["userInfo"]),
  },
  ...
  mounted() {
    ...
    this.templateForm.cluster_type = this.$route.params.type;
  },
  methods: {
    // //创建版本
    getTemInfo() {
      ...
      templatesInformation(this.$route.params.id).then((res) => {
        ...
        if (this.templateForm.cluster_type == "k8s") {
          this.$refs.addTemplateVersions.versionForm.template_id = res.id;
          this.$refs.addTemplateVersions.versionForm.owner = res.owner;
        } else {
          this.$refs.addK3sTemplateVersions.versionForm.template_id = res.id;
          this.$refs.addK3sTemplateVersions.versionForm.owner = res.owner;
        }
      });
    },
    // 模板数据初始化
    templatesInit(value) {
      ...
      this.templateForm.cluster_type = value.cluster_type;
    ...
    async getTemplates() {
      ...
      if (this.$refs.addTemplateVersions.versionForm.version) {
        this.loading = true;
        await getTemplates(this.templateForm)
          .then((res) => {
            if (this.templateForm.cluster_type == "k8s") {
              ...
              this.$refs.addK3sTemplateVersions.versionForm.template_id = res;
              this.$refs.addK3sTemplateVersions.versionForm.owner = this.userId;
              this.$refs.addK3sTemplateVersions.onSubmit();
            }
            this.loading = false;
          })
          .catch((err) => {
        ...
        this.$notify({
          title: this.$t("cluster.pleaseInputVersion"),
          type: "warning",
          duration: 2500,
        });
      }
    ...
    onSubmit() {
      if (this.templateForm.cluster_type == "k8s") {
        ...
        if (this.$refs.addK3sTemplateVersions.yamlLintState) {
          this.$notify({
            title: this.$t("cluster.clusterConfigYamlFormatError"),
            type: "error",
            duration: 2500,
