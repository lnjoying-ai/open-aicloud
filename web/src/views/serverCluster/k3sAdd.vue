<template>
  <div>
    ...
    <!-- <K3SinfoForm ref="infoRef"></K3SinfoForm> -->
    <el-card class="box-card mt-2 mb-2" shadow="never">
...
export default {
  ...
  methods: {
    ...
    delNode(index) {
      // 删除 节点编排计划
      this.$confirm(this.$t("cluster.confirmDelete"), this.$t("form.tip"), {
    ...
    onSubmit() {
      if (this.clusterForm.name == "") {
        this.$notify({
          title: this.$t("cluster.pleaseInputClusterName"),
          type: "error",
          duration: 2500,
        });
        return;
    ...
    submitCluster() {
      ...
      if (this.nowRegions == "" || this.nowSite == "") {
        ...
        this.clusterForm.k3s_config.k3s_version = k3s_config.k3s_version;
      }
      ...
      data.cluster_type = "k3s"; // k8s or k3s
      ...
      if (infoFrom.create_type == "import") {
        ...
        data.k3s_config = yaml.load(yamlJson);
      } else {
        data.k3s_config = k3s_config;
      }
      if (infoFrom.isUse) {
        // 是否使用模板
        data.tmpl_ver_id = infoFrom.version[1]; // 模板 版本选择
        delete data.k3s_config;
      }
      console.log(this.clustergpu);
      if (!this.clustergpu) {
        delete data.dev_needs[0].dev_spec.gpu;
      }
      console.log(data);
      postClusters(
