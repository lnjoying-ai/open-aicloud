<template>
  <div class="warpMain">
    ...
    <div class="mt-2">
      ...
      >
        <el-table-column
          prop="service_display_name"
          :label="$t('form.resourceService')"
        >
          <template slot-scope="scope">
            {{
              scope.row.service_display_name
                ? scope.row.service_display_name
                : "-"
            }}
          </template>
        ...
        <el-table-column
          prop="action_total"
          :label="$t('form.actionCount')"
          sortable
        />
        <el-table-column
          prop="condition_total"
...
<script>
import { getResource, delResource } from "@/api/iam/resource";
...
export default {
  ...
  methods: {
    ...
    async init() {
      ...
      const list = await getResource(this.queryData);
      this.tableData = list;
    },
    clickAddBtn() {
      this.$router.push({
    ...
    clickDelBtn(value) {
      this.$confirm(
        this.$t("message.confirmDeleteResource"),
        this.$t("message.tip"),
        {
