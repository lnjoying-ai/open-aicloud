  ...
  <div class="mt-2">
    <el-card class="mb-2" shadow="never">
      ...
      >
        ...
        <el-form-item :label="$t('form.email')+ ':'" prop="contact_info.email">
          <span v-if="detailMain.contact_info">
            {{
              detailMain.contact_info.email
                ? detailMain.contact_info.email
                : "-"
            }}
          </span>
        </el-form-item>
        <el-form-item :label="$t('form.phone')+ ':'" prop="contact_info.phone">
          <span v-if="detailMain.contact_info">
            {{
              detailMain.contact_info.phone
                ? detailMain.contact_info.phone
                : "-"
            }}
          </span>
        </el-form-item>
        <el-form-item :label="$t('form.belongOrganization')+ ':'" prop="bp_name">
          <span>{{ detailMain.bp_name ? detailMain.bp_name : "-" }}</span>
        </el-form-item>
        <el-form-item :label="$t('form.status')+ ':'">
          <span v-if="detailMain.status == 0">
            <el-tag size="small" type="info">{{
              $t("form.pendingActivation")
            }}</el-tag>
          </span>
          <span v-if="detailMain.status == 1">
            <el-tag size="small" type="success">{{
              $t("form.normal")
            }}</el-tag></span>
          <span v-if="detailMain.status == 2">
            <el-tag size="small" type="warning">{{
              $t("form.overdue")
            }}</el-tag></span>
          <span v-if="detailMain.status == 3">
    ...
    <el-card class="mb-2">
      ...
      <el-row :gutter="40">
        <el-col :span="24" style="clear: both">
          ...
          >
            <el-tab-pane :label="$t('form.empoweredPolicy')" name="first">
              <el-table
                ref="multipleTable"
                :data="policieData"
            ...
            <el-tab-pane :label="$t('form.empoweredRole')" name="second">
              <el-table
                ...
                max-height="600"
              >
                <el-table-column prop="role_name" :label="$t('form.name')">
                  <template slot-scope="scope">
                    <router-link
                      style="color: #409eff"
    ...
    >
      <empowerVue
        ...
        :sup_this="sup_this"
        :is-add="false"
        :authorize-policy="policieData"
        :authorize-role="roleData"
      />
...
import { detailUsers, editUsers } from '@/api/iam/user'
...
export default {
  ...
  components: { YamlEditor, empowerVue, operate },
  watch: {},
  data() {
    return {
      sup_this: this,
  ...
  methods: {
    async init() {
      const list = await detailUsers(this.$route.params.id)
    ...
    // 策略列表
    async initPolicie() {
      this.queryPoliceData.user_id = this.$route.params.id
      const list = await getAuthz(this.queryPoliceData)
      this.policieData = list.attachments
      this.queryPoliceData.total = list.total_num
    },
    async initRole() {
      this.queryData.user_id = this.$route.params.id
      const list = await getRolesAuthz(this.queryData)
      this.roleData = list.attachments
    ...
    async expandChange(row) {
      // console.log(this.$refs.yamlEditor)
      ...
      this.policieMain.document.statement.forEach((el, index) => {
        index == this.policieMain.document.statement.length - 1
    ...
    clickUnbindRoleBtn(value) {
      this.$confirm(
        this.$t('message.confirmCancelEmpower'),
        this.$t('message.tip'),
        {
