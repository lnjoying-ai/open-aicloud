<template>
  <div class="warpMain">
    ...
    <el-table :data="tableData.stacks" style="width: 100%">
      <el-table-column
        :label="$t('form.name')"
        prop="name"
      ...
      <el-table-column
        :label="$t('form.applicationTemplate')"
        show-overflow-tooltip
      >
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('form.version')" width="90">
        <template slot-scope="scope">
      ...
      <el-table-column :label="$t('form.serviceNumber')" sortable>
        <template slot-scope="scope">
          {{ scope.row.service_ids ? scope.row.service_ids.length : 0 }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('form.region')" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.dst_node.dst_region_name || "-" }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('form.site')" show-overflow-tooltip>
        <template slot-scope="scope">
          {{ scope.row.dst_node.dst_site_name || "-" }}
        </template>
      </el-table-column>
      <el-table-column :label="$t('form.node')" show-overflow-tooltip>
        <template slot-scope="scope">
    ...
    >
      <div v-if="appConfig">
        <el-form label-width="131px">
          ...
          <el-form-item label="justice_compose:">
            <yaml-editor
              v-if="dialogVisible"
              ref="yamlEditor"
              ...
              style="
                ...
                min-width: 100%;
              "
              :download-name="appConfig.name + '-justice_compose'"
              :download-type="'yml'"
              :readonly="'cursor'"
            />
          </el-form-item>
...
export default {
  ...
  created() {
    ...
    this.$route.meta.smalltitle =
      this.$t("form.deploy") + "：" + this.$route.params.name;
  },
  mounted() {
  ...
  methods: {
    ...
    async init() {
      const list = await getDeploysSystemInstances(
    ...
    clickstartBtn(item) {
      // 启动
      this.$confirm(this.$t("form.confirmBootUp"), this.$t("form.tips"), {
    ...
    clickRestartBtn(item) {
      // 重启
      this.$confirm(this.$t("form.confirmRestart"), this.$t("form.tips"), {
        confirmButtonText: this.$t("form.confirm"),
        cancelButtonText: this.$t("form.cancel"),
        type: "warning",
      })
    ...
    clickstopBtn(item) {
      // 停止
      this.$confirm(this.$t("form.confirmStop"), this.$t("form.tips"), {
        confirmButtonText: this.$t("form.confirm"),
        cancelButtonText: this.$t("form.cancel"),
        type: "warning",
      })
