            ...
            <el-col :span="24">
              <el-form-item>
                ...
                <el-input
                  ...
                  :placeholder="$t('transfer.fileNamePlaceholder')"
                />
              </el-form-item>
              <el-form-item>
                <el-button
      ...
      <div class="breadcrumbMain">
        <el-breadcrumb separator="/">
          <el-breadcrumb-item
            ...
            :key="index"
          >
            <span class="cursor-pointer" @click="rootdirectory(index)">{{
              item == "" ? $t("transfer.rootDirectory") : item
            }}</span>
          </el-breadcrumb-item>
        </el-breadcrumb>
      </div>
          ...
          <template slot-scope="scope">
              ...
              >
                <template slot="append">
                  <i
                    ...
                    @click="toAddFile(scope.row)"
                  />

                  <i
                    class="iconfont icon-chacha iconClose"
                    @click="closeAddFile(scope.row)"
                  />
                </template>
              </el-input>
            </div>
            ...
            >
              <div class="overflow-ellipsis">
                ...
                />
                <span
                  :class="scope.row.type == 1 ? 'cursor-pointer' : ''"
  ...
  methods: {
    ...
    async getFile() {
      ...
      await transmissionBrowse(
        "",
        {
          ...
          publicRoot: this.transmissionFsStoreData.publicSpaces[0].path,
        },
        {
    ...
    async gettransInfo() {
      await transmissionFsStore("CPU")
        .then((res) => {
          this.transmissionFsStoreData = res;
    ...
    downloadfile(list) {
      ...
      transmissionDownload({
        ...
        root: this.transmissionFsStoreData.publicSpaces[0].path,
        storeNameSpace: {
          storeNameSpace:
            this.transmissionFsStoreData.storeInfo[0].storeNameSpace,
          netEntityInfo:
            this.transmissionFsStoreData.storeInfo[0].netEntityInfo,
        }, // 存储空间的信息文件上传的存储空间的信息
      })
        .then((res) => {
          this.$notify({
    ...
    fileclick(list) {
      ...
      list.forEach((item) => {
        transmissionUpload({
          ...
          storeBack: this.transmissionFsStoreData.storeInfo[0].storeNameSpace,
        })
          .then((res) => {
            this.$notify({
